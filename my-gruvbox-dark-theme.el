;;; my-gruvbox-dark-theme.el --- Gruvbox Dark theme with OCaml support -*- lexical-binding: t; -*-

;;; Commentary:
;;
;; Gruvbox Dark Emacs theme using morhetz's canonical palette
;; (#282828).  Comprehensive face coverage including tuareg, merlin,
;; and utop faces for OCaml development.

;;; Code:

(require 'my-theme-palette)

(deftheme my-gruvbox-dark "Gruvbox Dark theme with OCaml support.")

(defgroup my-gruvbox-dark-theme nil
  "Faces for my-dark theme."
  :group 'faces)

(defcustom my-gruvbox-dark-scale-org-headlines t
  "Whether to scale Org headlines."
  :type 'boolean
  :group 'my-gruvbox-dark-theme)
...
(let* ((palette (my-theme-palette 'gruvbox-dark))
  ...
  (custom-theme-set-faces
   ...
   `(isearch-fail ((t (:background ,(funcall p 'bg-paren-mismatch) :foreground ,(funcall p 'fg)))))
   ...
   `(my-org-active ((t (:foreground ,(funcall p 'org-active-fg) :weight bold))))
   `(my-org-waiting ((t (:foreground ,(funcall p 'org-waiting-fg) :weight bold))))
   `(my-org-cancelled ((t (:foreground ,(funcall p 'org-cancelled-fg) :weight bold :strike-through t))))
   ...
   `(org-agenda-structure ((t (:foreground ,(funcall p 'keyword) :weight bold))))
   `(org-agenda-date ((t (:foreground ,(funcall p 'keyword)))))
   `(org-agenda-date-today ((t (:foreground ,(funcall p 'keyword) :weight bold :slant italic))))
   ...
   `(show-paren-mismatch ((t (:background ,(funcall p 'bg-paren-mismatch) :foreground ,(funcall p 'fg) :weight bold))))
   ...
   ;; ---- Avy ----
   `(avy-lead-face ((t (:foreground ,(funcall p 'bg) :background ,(funcall p 'error)))))
   `(avy-lead-face-0 ((t (:foreground ,(funcall p 'bg) :background ,(funcall p 'keyword)))))
   `(avy-lead-face-1 ((t (:foreground ,(funcall p 'bg) :background ,(funcall p 'success)))))
   `(avy-lead-face-2 ((t (:foreground ,(funcall p 'bg) :background ,(funcall p 'warning)))))
...
(provide-theme 'my-gruvbox-dark)
;;; my-gruvbox-dark-theme.el ends here
